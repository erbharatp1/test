 ...
 */
@Component
public class BookingAdaptor implements Adaptor<BookingDTO, Booking> {
	@Autowired
	private PassengerAdaptor adaptor;

	@Override
	public List<Booking> uiDtoToDatabaseModelList(List<BookingDTO> uiobj) {

		return uiobj.stream().map(item -> uiDtoToDatabaseModel(item)).collect(Collectors.toList());
	}

	@Override
	public List<BookingDTO> databaseModelToUiDtoList(List<Booking> dbobj) {

		return dbobj.stream().map(item -> databaseModelToUiDto(item)).collect(Collectors.toList());
	}

	@Override
	public Booking uiDtoToDatabaseModel(BookingDTO uiobj) {
		...
		booking.setPrice(uiobj.getPrice());
		...
		booking.setPassengerList(adaptor.uiDtoToDatabaseModelList(uiobj.getPassengerList()));
		return booking;
	}

	@Override
	public BookingDTO databaseModelToUiDto(Booking booking) {
		...
		dto.setPnrNo(booking.getPnrNo());
		dto.setPrice(booking.getPrice());
		dto.setTicketStatus(booking.getTicketStatus());
		dto.setTrip(booking.getTrip());
		List<Passengers> pass = booking.getPassengerList();

		dto.setPassengerList(adaptor.databaseModelToUiDtoList(booking.getPassengerList()));
